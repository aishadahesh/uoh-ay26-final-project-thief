...
- **Limitation — no live match log yet:** `LogEntry`/`audit_log` are fully tested in isolation and via a realistic synthetic multi-turn sequence (`test_multi_turn_log_audit_catches_a_post_hoc_tampering_attempt`), but no *live* match currently produces a `LogEntry` sequence — `run_local_match` (Chapter 3/4) has no crypto layer wired in. That wiring, plus the Log Manager that would actually record entries turn-by-turn, is Chapter 8.
