...
- **Decision:** build **one shared package** (`police_thief`) during development, with the two roles differentiated at runtime by a `--role cop`/`--role thief` CLI flag and by loading separate `config/cop/`/`config/thief/` directories. Two OS processes running this same on-disk package never share memory — each gets its own interpreter and heap — so the "no shared runtime state" rule is satisfied without needing two copies of the source tree. The literal two-GitHub-repos deliverable is produced later, at submission time (`docs/TODO.md` §O), by exporting this single codebase into two tagged repos.
...
- **Status:** Accepted and executed for this repo. This repository is now finalized as the **thief** submission repo: `serve` defaults to `--role thief`, `config/cop/` is retained only as a local opponent-peer for interop testing (never a submission-grade cop config), and a separate sibling repo carries the real cop submission (README.md links it). §11's per-repo layout still describes the target shape each exported repo has at submission time.
