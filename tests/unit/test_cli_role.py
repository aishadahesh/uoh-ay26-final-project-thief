...
support in this repository"). `serve` must default to `--role thief`, and
`--role cop` must still parse (it runs a local opponent peer for interop
testing) but must announce, every time, that it isn't a supported
submission role here.
"""
